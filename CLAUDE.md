...
- **Headcount = approved members present**: presence-based headcounts (drinks/meals charts, meals NoOrg subtraction) must run on logistics rows pre-filtered through `JH.approvedLogistics(logistics, members)`, which keeps only rows whose `MemberName` matches an *approved* member (legal or playa name) and dedupes **by member identity** — a member with two logistics rows (one under legal name, one under playa name) counts once, not twice. Raw `MemberLogistics` includes observers, pending applicants, stale rows, guests, and legal/playa double-entries, so counting it directly over-reports (e.g. 31 present vs 29 approved). Pages fetch logistics then assign `state.logistics = JH.approvedLogistics(...)` so every downstream count is approved-only.
