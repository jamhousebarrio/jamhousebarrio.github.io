...
- **Invite link paths are status-gated**: every path that mints a portal link checks the *target's* status is Approved/Observer. The automatic paths gate via `shouldInvite`/`PORTAL_STATUSES`; the three **manual admin** actions in `api/auth.js` (`invite`, `resend-invite`, `generate-link`) gate via `assertPortalEligible(sheets, spreadsheetId, email)` in `/api/_lib/invite.js` — throws `403` (not Approved/Observer) or `404` (no application), caught by the handler's outer try/catch (not logged to ErrorLog, since these are expected denials). This prevents inviting a screening-stage member (e.g. "Vibe Check"): their link would expire and the self-serve magic-link/reset path — already status-gated in `handlePublicAuthEmailAction` — would correctly refuse to renew it, stranding them. `admin-applications.js` `sendInvite` surfaces the `403` message and skips the manual-link fallback (it's gated too). Unit-tested via `npm test`. The path to invite someone is: approve them first.
