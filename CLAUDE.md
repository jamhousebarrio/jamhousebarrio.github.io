...
- **Headcount = approved members present**: presence-based headcounts (drinks/meals charts, meals NoOrg subtraction) must run on logistics rows pre-filtered through `JH.approvedLogistics(logistics, members)`, which keeps only rows whose `MemberName` matches an *approved* member (legal or playa name) and dedupes **by member identity** — a member with two logistics rows (one under legal name, one under playa name) counts once, not twice. Raw `MemberLogistics` includes observers, pending applicants, stale rows, guests, and legal/playa double-entries, so counting it directly over-reports (e.g. 31 present vs 29 approved). Pages fetch logistics then assign `state.logistics = JH.approvedLogistics(...)` so every downstream count is approved-only. The legal/playa double-rows are also prevented at the source: `api/logistics.js` `upsert` matches a member's existing row under **either** of their names before deciding insert-vs-update, so a name change no longer orphans the old row. Existing dupes are cleaned by the one-shot `scripts/merge-logistics-dupes.mjs` (dry-run default; `--apply` to write).
...
- **Invite link paths are status-gated**: every path that mints a portal link checks the *target's* status is Approved/Observer. The automatic paths gate via `shouldInvite`/`PORTAL_STATUSES`; the three **manual admin** actions in `api/auth.js` (`invite`, `resend-invite`, `generate-link`) gate via `assertPortalEligible(sheets, spreadsheetId, email)` in `/api/_lib/invite.js` — throws `422` (not Approved/Observer) or `404` (no application), caught by the handler's outer try/catch (not logged to ErrorLog, since these are expected denials). The ineligible case is **422, not 403, on purpose**: the shared `JH.apiFetch` treats `401`/`403` as a session-auth failure and force-redirects to `/admin`, so a 403 would bounce the admin to the login page on what is really a per-target validation error — `422`/`404` flow back to the caller instead. This prevents inviting a screening-stage member (e.g. "Vibe Check"): their link would expire and the self-serve magic-link/reset path — already status-gated in `handlePublicAuthEmailAction` — would correctly refuse to renew it, stranding them. `admin-applications.js` `sendInvite` surfaces the `422`/`404` message and skips the manual-link fallback (it's gated too). Unit-tested via `npm test`. The path to invite someone is: approve them first.
...
- **If you add a new `Kind` to ShiftWeights** → update `buildWeightIndex` in `shift-points-logic.js` AND the full-replace delete/append logic in `api/shifts.js` `set-weights` (each `Kind` is wiped+re-appended independently; an un-handled kind is silently dropped or never persisted). The `set-weights` role-replace is guarded on `Array.isArray(roles)` so callers that omit `roles` (e.g. `cleanupWeightsAfterDelete`) don't wipe role rows.
...
| ShiftWeights | shifts.js | Point weights for the fairness leaderboard. Cols: Kind (type/role/build/strike), Name (type or role name; blank for build/strike), Points. Defaults applied in shift-points-logic.js: type=1, role=10, build/strike=10. |
...
> Shift point weights live in the ShiftWeights tab and are set via the admin-only "⚖ Points" modal on the Shifts page (`set-weights` action; `get-weights` is open to any authed user since the leaderboard needs it). Points replace clock hours as the leaderboard's ranking currency; hours remain a displayed detail. Build/strike days earn points per day present, **minus** any NoOrg days in that member's build/strike window. The **arrival day earns no build points** and the **departure day earns no strike points** (treated as travel/arrival, not work) — build counting starts the day *after* arrival, strike counting ends the day *before* departure, so arriving the day before the event (or leaving the day after) contributes 0 build/strike days. The personal modal still lists those boundary days, tagged "· no points". **Lead/role points**: every role in the Roles tab earns points for its assignees (default **10**/role, admin-editable per role in the same ⚖ Points modal; set 0 to exclude a non-lead role). A member's role points are the **sum** across all roles they hold — role assignment is matched by playa **or** legal name against the Roles `AssignedTo` (same rule as `api/_lib/roles.js` `isAssignedToRole`, mirrored client-side as `rolesForMember` in `admin-shifts.js`). Note a name like "Shit Ninja" can be both a role and a shift type; they score independently. The personal modal shows per-line math (build/strike days × weight = pts, each role's pts, each shift's pts) and a grand **Total**, all sourced from the same `memberPoints()` the leaderboard uses. Pure scoring math (incl. defaults type=1, role=10, build/strike=10) is in `assets/js/shift-points-logic.js`, unit-tested via `npm test`.
